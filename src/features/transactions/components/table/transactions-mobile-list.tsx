...
function TransactionMobileRow({
	...
	const isOverdue =
		isBoleto && !item.isSettled && new Date(dueDate) < new Date();
	...
	const accountContent = (
		<div className="flex items-center gap-2 min-w-0">
			...
			<span
				className={cn(
					"truncate",
					isOwnData && accountHref && "underline-offset-2",
				)}
			>
				{accountLabel}
			</span>
			...
			<div className="min-w-0 flex-1 flex flex-col gap-1">
				...
				<div className="flex items-center gap-2 text-xs text-muted-foreground min-w-0">
					{isOwnData && accountHref ? (
						...
						<div className="min-w-0 truncate">{accountContent}</div>
					)}
				</div>
					...
					<div className="flex items-center gap-2 text-xs text-muted-foreground min-w-0">
						{(payerId || pagadorName) &&
							(payerId ? (
								...
								<div className="min-w-0 truncate">{payerContent}</div>
							))}
						{(payerId || pagadorName) && dueDateLabel && (
